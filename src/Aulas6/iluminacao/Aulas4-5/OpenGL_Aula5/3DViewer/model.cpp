...
void Model::createNormals(){
    ...
    for (int i = 0; i < numFaces; i++) {
        //calculate normal for every triang
        unsigned int a = indices[i * 3 + 0];
        unsigned int b = indices[i * 3 + 1];
        unsigned int c = indices[i * 3 + 2];

        QVector3D v1 = vertices[a].toVector3D();
        QVector3D v2 = vertices[b].toVector3D();
        QVector3D v3 = vertices[c].toVector3D();

        //acumulate in every normal connected
